...
test('new Node(string)', () => expect(new Node('/a/b/c')).toBeDefined());

test('new Node(array)', () => expect(new Node(['a', 'b', 'c'])).toBeDefined());

test('Node singleton by path', () =>
  expect(new Node('/a/b/c')).toStrictEqual(new Node(['a', 'b', 'c'])));
...
test('Node.prototype.name', () => {
  ...
  expect(new Node(__filename).name()).toBe(testFile);
});
...
test('Node.prototype.parent', () => {
  ...
  expect(currentDirectory).toStrictEqual(new Node(__dirname));
});
...
test('Node.prototype.resolve(string)', () => {
  ...
  expect(currentFile.resolve('..')).toStrictEqual(currentDirectory);
});
...
test('Node.prototype.newFile(string, [string])', () => {
  ...
  expect(() => cwd.getContent()).toThrowError();
});

test('Node.prototype.children', () => {
  expect(generatedDirectory.children()).toEqual([
...
test('Node.prototype.clear', () => {
  ...
  child.clear();
  expect(child.getContent()).toBe('');

  generatedDirectory.clear();
  expect(generatedDirectory.children()).toEqual([]);

  expect(() => child.clear()).toThrowError();
});

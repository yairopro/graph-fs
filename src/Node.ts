...
class Node {
  ...
  constructor(absolutePath: string | string[]) {
    ...
    if (typeof absolutePath === 'string') {
      ...
      if (!absolutePath[0]) absolutePath = absolutePath.slice(1);
      // remove empty string at end
      const lastNodeName = lastItemOf(absolutePath);
      if (!lastNodeName) absolutePath = absolutePath.slice(0, -1);
    }
  ...
  extension(): string | undefined {
    if (this.is().file) {
      ...
      if (splittedName.length > 1) return lastItemOf(splittedName);
    }
    return undefined;
  }
  ...
  newDirectory(name: string): Node {
    ...
    if (directory.exists()) throw new Error('Directory already exists.');
  ...
  newFile(
    fullName: string,
    content: any = '',
    options: any | undefined = 'utf8'
  ): Node {
    const file = this.resolve(fullName);

    if (file.exists()) throw new Error('File already exists.');
  ...
  children(): Node[] {
    ...
    return fs.readdirSync(absolute).map(name => this.resolve(name));
  }
  ...
  copy(to: string) {
    ...
    if (this.is().file) fs.copyFileSync(this.absolute(), dest.absolute());
    else if (this.is().directory) {
      dest.parent().newDirectory(dest.name());
      this.children().forEach(child =>
        child.copy(path.resolve(dest.absolute(), child.name()))
      );
    }
   ...
   */
  clear() {
    if (!this.exists()) throw new Error(`Node does not exist: ${this}`);

    const is = this.is();
    if (is.file) fs.writeFileSync(this.absolute(), '', 'utf8');
    else if (is.directory) this.children().forEach(child => child.delete());
  }
